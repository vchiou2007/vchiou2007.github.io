...
test('實際 App 啟動、飛花令路由、遮蔽和搜尋不影響舊路由',async()=>{
 ...
 try{
  ...
  location.hash='#/my-recitation';windows.hashchange();input.id='recitation-search';input.value='床前明月光';for(const fn of handlers.input)fn({target:input,isComposing:false});assert.ok(app.innerHTML.includes('符合 1 項'));assert.ok(app.innerHTML.indexOf('poetic-welcome')<app.innerHTML.indexOf('recitation-table'));
  location.hash='#/today';windows.hashchange();assert.ok(app.innerHTML.indexOf('poetic-welcome')<app.innerHTML.indexOf('recitation-table'));
  location.hash='';windows.hashchange();assert.ok(app.innerHTML.includes('<h1>我的背誦'));assert.ok(app.innerHTML.includes('href="#/my-recitation" aria-current="page"'));assert.ok(app.innerHTML.indexOf('poetic-welcome')<app.innerHTML.indexOf('recitation-table'));
