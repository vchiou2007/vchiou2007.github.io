...
test('1,002 組兩句選摘都有完整原文，沒有空白或越界',()=>{
 const pairs=pairEntries(poems);assert.equal(pairs.length,1002);assert.equal(new Set(pairs.map(q=>q.id)).size,1002);
...
test('擴充後保留舊備份與新作品收藏，所有作品可出現在總覽',()=>{
 ...
 const html=catalogueView(poems,state,{query:'',type:'',theme:'',sort:'default',selected:[]});assert.ok(html.includes('501 首'));assert.ok(!html.includes('undefined'));
 const pairs=pairsView(poems,state,{query:'',type:'',theme:'',saved:false});assert.ok(pairs.includes('1002 組'));assert.ok(!pairs.includes('undefined'));
});
